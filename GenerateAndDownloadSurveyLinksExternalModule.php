...
class GenerateAndDownloadSurveyLinksExternalModule extends AbstractExternalModule
{
    ...
    function generateDocumentSurveyLinksCron($cronAttributes){
        if(self::isCronInHourRange($cronAttributes['cron_name'])) {
    ...
    function isCronInHourRange($thisCron){
        $hourRange = 6;
        if(date('G') > $hourRange){
        ...
        if(!self::checkCronStatus($hourRange) || !self::checkCronStatus($hourRange, $thisCron)){
            return false;
        }
    ...
    function checkCronStatus($hourRange, $thisCron = ""){
        if(empty($thisCron)){
            $lastRunSettingName = 'last-cron-run-time';
        }else{
            $lastRunSettingName = 'last-cron-run-time-'.$thisCron;
        }
